...
//评论动态(必传token, composeId: 动态id   ): http://192.168.101.10:4000/user/comment/createTrendsComment?content=&image=&composeId=
commentfn(){
  var that = this;
  wx.request({
    url: app.globalData.myhost + '/user/comment/createTrendsComment',
    method: 'POST',
    header: {
...
//回复动态评论(必传token,必传 commentId为评论id): http://192.168.101.10:4000/user/reply/createToTrendsComment?content=&image=&commentId=
  replyCommentfn() {
    var that = this;
    wx.request({
      url: app.globalData.myhost + '/user/reply/createToTrendsComment',
      method: 'POST',
      header: {
        'Content-Type': 'application/x-www-form-urlencoded',
        token: `${JSON.parse(wx.getStorageSync('token'))}`,
    ...
    //回复动态回复(必传token, 必传 replyId为回复id): http://192.168.101.10:4000/user/reply/createToTrendsReply?content=&image=&replyId=
  replyCommentreplyfn() {
    var that = this;
    wx.request({
      url: app.globalData.myhost + '/user/reply/createToTrendsReply',
      method: 'POST',
      header: {
        'Content-Type': 'application/x-www-form-urlencoded',
        token: `${JSON.parse(wx.getStorageSync('token'))}`,
